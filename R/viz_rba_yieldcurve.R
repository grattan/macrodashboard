...
viz_rba_yieldcurve <- function(data = load_data(),
  ...
  df %>%
    ggplot(aes(x = .data$years_to_maturity,
               y = .data$value,
               col = .data$date_type)) +
    geom_line() +
    grattan_point_filled(data = ~group_by(., .data$date_type) %>%
                 filter(.data$years_to_maturity == max(.data$years_to_maturity)),
               size = 3, fill = "white") +
    grattan_label_repel(data = ~group_by(., .data$date_type) %>%
                          filter(.data$years_to_maturity == max(.data$years_to_maturity)),
                        ...
                        size = 16,
                        nudge_x = 0.2) +
    scale_y_continuous(limits = c(min(0, min(df$value)),
                                    max(6, max(df$value))),
                         labels = function(x) paste0(x, "%")) +
    scale_x_continuous(expand = expansion(add = c(0, 4)),
                       breaks = seq(0, 30, 10),
