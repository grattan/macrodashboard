...
viz_emp_agesex_recessions <- function(data = load_data(),
  ...
  df <- df %>%
    filter(series_id %in% c("A84424191X",
                             "A84424163R",
                             "A85255850V",
                             "A85255790C",
                             "A85255778L",
  ...
  df <- df %>%
    mutate(age_group = case_when(age == "15-24 years" ~ "15-24",
                                 age %in% c("25-34 years",
                                            "35-44 years",
                                            "45-54 years") ~ "25-54",
                                 age == "55 years and over" ~ "55+")) %>%
    group_by(date, sex, age_group) %>%
    summarise(emp = sum(value))
  ...
  tps <- tibble::tribble(
    ...
    bind_rows(tibble(peak_date = as.Date("2020-02-01"),
                     ...
                     recession = "COVID")) %>%
    mutate(min_date = peak_date + lubridate::years(3))
  ...
  df <- df %>%
    arrange(date) %>%
    group_by(sex, age_group, recession) %>%
    mutate(months_since_start = row_number() - 1,
           change_since_start = (emp / emp[date == min(date)]) - 1)
  ...
  make_plot <- function(.sex) {
    ...
    plot <- df %>%
      filter(sex == .sex) %>%
      ggplot(aes(x = months_since_start, y = change_since_start, col = recession)) +
      ...
      scale_y_continuous(labels = function(x) paste0(x * 100, "%"),
                         ...
                         limits = c(-0.2, 0.2)) +
      labs(subtitle = .sex) +
      theme(plot.subtitle = element_text(size = 18, colour = "black",
                                         hjust = 0.54,
                                         vjust = 0,
    ...
    if (.sex == "Women") {
      plot +
        ggtext::geom_richtext(
          data = tibble(age_group = "15-24",
                        x = c(0, 12),
                        y = .05,
                        label = c("<span style='color:#FFC35A'>1980s</span><br><span style='color:#F68B33'>1990s</span>",
  ...
  make_plot("Women") +
    ...
    plot_annotation(title = "Youth employment fell sharply with the COVID shock",
