...
viz_payroll_bysex <- function(data = load_data(),
  ...
  df %>%
    ...
    grattan_label_repel(data = ~filter(., .data$date == max(.data$date)),
    ...
    scale_x_date(expand = expansion(mult = c(0, 0.17)),
                 breaks = seq.Date(from = as.Date("2020-03-14"), to = max(df$date) + 30,
                          length.out = 5),
                 date_labels = "%b") +
    theme_grattan() +
    theme(axis.title = element_blank() ) +
    labs(title = "Women lost jobs quicker, but rebounded more than men",
