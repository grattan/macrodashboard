...
viz_payrolls_bystate <- function(data = load_data(),
  ...
  df <- df %>%
    dplyr::filter(.data$industry == "All industries",
                  .data$sex == "Persons",
                  .data$age == "All ages",
                  .data$state %in% states,
                  .data$date >= as.Date("2020-03-14"))

  cols <- c("Australia" = "black",
  ...
  title <- dplyr::case_when(
    ...
    any(latest$value < 100) ~ paste0("The total ", series_desc, " is still below March levels in ",
