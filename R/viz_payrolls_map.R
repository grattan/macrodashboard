...
viz_payrolls_map <- function(data = load_data(),
  ...
  df <- df %>%
    mutate(city = case_when(grepl("Perth", .data$sa4) ~ "Perth",
  ...
  df %>%
    ...
    coord_sf() +
    labs(title = paste(worst_area,
                       "is the hardest-hit area in",
                       city,
                       sep = " "),
         subtitle = paste0("Change in the number of payroll jobs between",
                           " 14 March 2020 and ",
