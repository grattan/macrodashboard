...
viz_apple_mobility <- function(data = load_data(),
  ...
  df <- df %>%
    mutate(
      transportation_type = tools::toTitleCase(.data$transportation_type),
  ...
  df %>%
    ...
    grattan_label(
      data = ~ filter(., .data$transportation_type == "Public transport") %>%
    ...
    scale_x_date(
      breaks = seq.Date(lubridate::ymd("2020-02-01"), max(df$date), length.out = 4L),
      date_labels = "%b",
    ...
    labs(
      title = "Movement has recovered, but is mostly still below pre-COVID levels",
