...
viz_forecasts_cpi_wpi <- function(data = load_data(),
  ...
  latest_text <- paste0(
    ...
    "the latest RBA forecast was issued in ", format(
      max(raw_forecasts$forecast_date),
      "%b %Y"
    ), "."
  )
  ...
  combined %>%
    ...
    geom_rect(
      ...
      inherit.aes = FALSE,
      aes(
        xmin = .data$min_date, xmax = .data$max_date,
        ymin = -Inf, ymax = Inf
      ),
    ...
    geom_text(
      data = tibble(
        series = "Inflation",
        x = c(
          stats::median(combined$date),
          mean(c(
            forecast_band$min_date,
