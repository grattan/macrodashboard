...
viz_hours_pop <- function(data = load_data(),
  ...
  hours <- data$lfs_m_19 %>%
    filter(
      .data$series == "Monthly hours worked in all jobs ;  Persons ;",
  ...
  hours_to_highlight <- hours %>%
    ...
    mutate(label = paste0(
      round(.data$hours_per_capita, 1), " in\n",
  ...
  hours %>%
    ...
    labs(
      title = "The number of hours worked per person fell dramatically after the COVID shock",
