...
viz_payrolls_byind_bar <- function(data = load_data(),
  ...
  chart_data <- df %>%
    filter(industry != "All industries",
           ...
           age == "All ages",
           date == max(date)) %>%
    mutate(industry = gsub("[[:alpha:]]-", "", industry))
  ...
  title <- case_when(highlight_level < 100 ~
                       paste0("About ", 100 - highlight_level, " per cent of ",
                              tolower(highlight_industry), " jobs have been lost since March 2020"),
                     highlight_level == 100 ~
                       paste0("The number of jobs in ", tolower(highlight_industry),
  ...
  chart_data %>%
    mutate(highlight = ifelse(industry == highlight_industry, TRUE, FALSE)) %>%
    ...
    labs(title = title,
         subtitle = paste0("Change in number of payroll jobs between 14 March 2020 and ",
