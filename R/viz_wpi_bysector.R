viz_wpi_bysector <- function(data = load_data(),
  ...
  df <- df %>%
    dplyr::filter(.data$series_type == "Seasonally Adjusted",
                  grepl("Percentage Change From Corresponding Quarter of Previous Year",
                        .data$series),
                  !is.na(value))
  ...
  df <- df %>%
    mutate(sector = case_when(.data$series_id == "A83895309L" ~ "Private",
  ...
  cols <- c("Private" = grattantheme::grattan_red,
  ...
  df %>%
    ...
    grattan_label_repel(data = latest,
    ...
    grattan_y_continuous(limits = c(min(0, min(df$value)),
                                    max(5, max(df$value))),
