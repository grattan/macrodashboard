viz_wpi_byind <- function(data = load_data(),
  ...
  df <- df %>%
    mutate(
      industry = gsub(
        "Percentage Change from Corresponding Quarter of Previous Year ;  Total hourly rates of pay excluding bonuses ;  Australia ;  Private and Public ;  ",
        "",
        .data$series
      ),
  ...
  df <- df %>%
    mutate(short_ind = case_when(
      industry == "Other services" ~ industry,
  ...
  df <- df %>%
    mutate(
      short_ind = gsub("and", "&", .data$short_ind),
  ...
  df <- df %>%
    ...
    summarise(
      latest = .data$value[.data$date == max(.data$date)],
      ave = mean(.data$value)
    ) %>%
    gather(
  ...
  df %>%
    ggplot(aes(
      x = reorder(.data$short_ind, .data$rank),
      y = .data$value,
      col = .data$series
    )) +
    geom_point(size = 3.5) +
    grattan_label_repel(
      data = ~ filter(., .data$rank == max(.data$rank)) %>%
